...
| `Load()` | Existing session file, or `.bak` if the primary file is absent | Restores project order, active project, graphs, base directories, counters, and workspace. |
...
`projectOrder`, and each project's ID, name, base directory, nodes, edges, their
order, and next node/edge counters. Undo/redo stacks are not restored. A removed
