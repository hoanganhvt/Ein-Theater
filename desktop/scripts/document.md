...
| `test.mjs` | Go and frontend source | Runs Go tests, Studio/Canvas JavaScript regressions, chooser tests, and Electron security checks. Uses `build/go-cache` for Go's cache. |
| `checksum.mjs` | Built NSIS installer | SHA-256 sidecar file for the installer. |
| `smoke-packaged.mjs` | `dist/win-unpacked/Ein Theater.exe` | Starts the packaged app with isolated temporary user data and a stubbed directory picker, asserts startup/UI/security contracts, and writes a screenshot to `build/`. |

Run `npm.cmd test` from `desktop/` for source checks. After `npm.cmd run dist:win`,
run `npm.cmd run smoke:packaged`. The packaged smoke uses a stubbed folder result:
it verifies selection wiring but does not exercise the real Windows dialog or
